<?php
    ...
    if(empty($boardTitle) || empty($boardContents)){
        ...
        $sql = "INSERT INTO board(boardID, memberID, boardTitle, boardContents, regTime, isPinned) VALUES('$boardID', '$memberID', '$boardTitle', '$boardContents', '$regTime', '$isPinned')";
        $result = $connect->query($sql);

        if($result){
            echo "<script>alert('게시글이 성공적으로 작성되었습니다.'); window.location.href = 'comm_notice.php';</script>";
        } else {
