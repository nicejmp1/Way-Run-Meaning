...
<?php
    // 보드뷰 + 1
    $sql = "UPDATE board4 SET boardView = boardView + 1 WHERE boardID = {$boardID}";
    $connect->query($sql);

    // 데이터 가져오기
    $sql = "SELECT b.boardTitle, m.youNickName, b.regTime, b.boardView, b.boardContents FROM board4 b JOIN members m ON(b.memberID = m.memberID) WHERE b.boardID = {$boardID}";
