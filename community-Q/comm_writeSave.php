<?php
    ...
    if(empty($boardTitle) || empty($boardContents)){
        ...
        $sqlLastID = "SELECT MAX(boardID) AS lastID FROM board4";
        ...
        $sqlDeleted = "SELECT boardID FROM board4 WHERE boardDelete = 0 ORDER BY boardID DESC LIMIT 1";
        ...
        $sql = "INSERT INTO board4(boardID, memberID, boardTitle, boardContents, regTime, isPinned) VALUES('$boardID', '$memberID', '$boardTitle', '$boardContents', '$regTime', '$isPinned')";
        $result = $connect->query($sql);

        if($result){
            echo "<script>alert('게시글이 성공적으로 작성되었습니다.'); window.location.href = 'comm_question.php';</script>";
        } else {
