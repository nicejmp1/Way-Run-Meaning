...
$sql = "SELECT b.boardID, b.boardTitle, m.youNickName, b.regTime, b.boardView FROM board3 b JOIN members m ON (b.memberID = m.memberID) ORDER BY boardID DESC LIMIT {$viewLimit}, {$viewNum}";
$result = $connect -> query($sql);
...
if($page !=1){
    $prevPage = $page - 1;
    echo "<li class='first'><a href='comm_meet.php?page=1'>처음으로</a></li>";
    echo "<li class='prev'><a href='comm_meet.php?page={$prevPage}'>이전</a></li>";
}
...
for($i=$startPage; $i<=$endPage; $i++){
    ...
    echo "<li class='{$active}'><a href='comm_meet.php?page={$i}'>{$i}</a></li>";
}

// 마지막으로 & 다음
if($page < $boardTotalCount){
    $nextPage = $page + 1;
    echo "<li class='next'><a href='comm_meet.php?page={$nextPage}'>다음</a></li>";
    echo "<li class='last'><a href='comm_meet.php?page={$boardTotalCount}'>마지막으로</a></li>";
}
?>
                        </ul>
                    </section>
